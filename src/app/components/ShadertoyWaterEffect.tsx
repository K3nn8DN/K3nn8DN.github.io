...
export function ShadertoyWaterEffect({ 
  ...
  const loadImage = useCallback((gl: WebGLRenderingContext, url: string) => {
    ...
    // Create a transparent placeholder while loading
    const canvas = document.createElement('canvas');
    canvas.width = 1;
    canvas.height = 1;
    const ctx = canvas.getContext('2d')!;
    ctx.clearRect(0, 0, 1, 1); // Transparent background
  ...
  const handleMouseMove = useCallback((e: React.MouseEvent<HTMLCanvasElement>) => {
    ...
    mouseRef.current.y = canvas.height - (e.clientY - rect.top) * dpr; // Flip Y coordinate and scale by DPR
  }, []);
